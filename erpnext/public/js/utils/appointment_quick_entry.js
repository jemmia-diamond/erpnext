...
frappe.ui.form.AppointmentQuickEntryForm = class AppointmentQuickEntryForm extends (
	...
	setup_custom_logic() {
		frappe.db
			.get_value("Sales Person", { employee_email: frappe.session.user }, ["name", "sales_person_name"])
			.then((r) => {
	...
	check_existing_appointment() {
		...
		if (!this.doc.customer_phone_number) {
			if (this.doc.lead) {
				get_phone_promise = frappe.db
					.get_value("Lead", this.doc.lead, ["mobile_no", "phone"])
					.then((r) => (r.message ? r.message.mobile_no || r.message.phone : null));
			} else if (this.doc.party && this.doc.appointment_with === "Customer") {
				get_phone_promise = frappe.db
					.get_value("Customer", this.doc.party, ["mobile_no", "phone"])
					.then((r) => (r.message ? r.message.mobile_no || r.message.phone : null));
			}
		}
		...
		get_phone_promise.then((phone) => {
			...
			if (this.doc.party && this.doc.appointment_with === "Customer")
				or_filters.push(["party", "=", this.doc.party]);
			if (phone) or_filters.push(["customer_phone_number", "=", phone]);

			if (or_filters.length === 0) return;

			frappe.db
				.get_list("Appointment", {
					filters: [
