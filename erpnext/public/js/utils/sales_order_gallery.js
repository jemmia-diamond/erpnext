...
$.extend(erpnext.utils.sales_order_gallery, {
	bind_gallery_listeners(frm) {
		...
		if ($grid && $grid.length) {
			const rerender = frappe.utils.debounce(
	...
	async render_gallery(frm) {
		...
		if (!list.length) {
			$wrap.append(
				'<div class="so-gallery"><div class="empty">Không có ảnh nào để hiển thị</div></div>'
			);
			return;
		}
		...
		$wrap.off("click.soimg").on("click.soimg", ".so-gallery img", (e) => {
			...
			const $img = $("<img>", { style: "display:block; max-width:100%; margin:auto;" }).attr(
				"src",
				src
			);
			$imgContainer.append($img);

			const $srcText = $('<div style="padding:8px 12px; color:#6b7280; font-size:12px;"></div>').text(
				src
			);
