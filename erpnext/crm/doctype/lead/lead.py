...
class Lead(SellingController, CRMNote):
	...
	if TYPE_CHECKING:
		...
		state: DF.Data | None
		status: DF.Literal[
			"New", "Prospecting", "Nurturing", "Qualified", "Converted", "Do Not Contact", "Spam"
		]
		store: DF.Link | None
	...
	def check_and_auto_create_opportunity(self):
		...
		if opp.expected_delivery_date and frappe.utils.getdate(
		...
		try:
			if self.pancake_data:
				...
				if latest_message_at:
					if not self.last_message_at or get_datetime(self.last_message_at) < get_datetime(
						latest_message_at
					):
