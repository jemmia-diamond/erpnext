...
def insert_lead(doc):
	...
	if doc.get("doctype") and frappe.is_table(str(doc.doctype)):
		if not (doc.parenttype and doc.parent and doc.parentfield):
			frappe.throw(_("Parenttype, Parent and Parentfield are required to insert a child record"))

		# inserting a child record
		parent = frappe.get_doc(str(doc.parenttype), str(doc.parent))
		parent.append(str(doc.parentfield), doc)
		parent.save()
		return parent
	...
	try:
		...
		if getattr(frappe_doc, "first_reach_at", None):
			dt_reach = get_datetime(str(frappe_doc.first_reach_at))
			dt_assign = get_datetime(config.DATE_ASSIGN_LEAD_OWNER)
			if dt_reach and dt_assign and dt_reach < dt_assign:
				try:
					create_lead_todo(str(frappe_doc.name), str(frappe_doc.lead_owner))
				except Exception as e:
					frappe.log_error(e)

		return frappe_doc
	except Exception:
		frappe.log_error(frappe.get_traceback(), "insert_lead failed")
		try:
			existing_doc = frappe.get_doc(frappe_doc.doctype, str(frappe_doc.name))
			if existing_doc:
				return existing_doc
			return None
		except Exception:
	...
	for doc in docs:
		...
		try:
			...
			try:
				...
				existing_doc = cast(Any, frappe.get_doc(str(doc["doctype"]), str(docname)))
				...
				if lead_name:
					existing_doc = cast(Any, frappe.get_doc(str(doc["doctype"]), lead_name))
				else:
					doc.pop("docname", None)
					existing_doc = cast(Any, insert_lead(doc))
			...
			if getattr(existing_doc, "phone", None) and existing_doc.phone != "":
				doc["phone"] = normalize_phone_number(str(existing_doc.phone))
			...
			if is_valid_lead_name(getattr(existing_doc, "lead_name", None)):
				doc["first_name"] = existing_doc.lead_name
				doc["lead_name"] = existing_doc.lead_name

			if getattr(existing_doc, "lead_owner", None):
...
def handle_duplicate_and_merge(existing_doc, new_phone):
	...
	existing_doc_lead = cast(Lead, existing_doc)
	...
	if existing_doc_lead.first_reach_at and conflicting_doc.first_reach_at:
		dt_exist = get_datetime(str(existing_doc_lead.first_reach_at))
		dt_conf = get_datetime(str(conflicting_doc.first_reach_at))
		if dt_exist and dt_conf and dt_exist < dt_conf:
			is_existing_older = True
	elif existing_doc_lead.first_reach_at:  # conflicting has no date
		is_existing_older = True

	if is_existing_older:
		master_doc = existing_doc_lead
		loser_doc = conflicting_doc
	else:
		master_doc = conflicting_doc
		loser_doc = existing_doc_lead
...
def process_bulk_merge(duplicate_phones):
	...
	"""
	for _idx, row in enumerate(duplicate_phones):
...
def process_fix_unnormalized(leads):
	for row in leads:
		...
		try:
			...
			if isinstance(e, (frappe.UniqueValidationError, pymysql.err.IntegrityError)):
				...
				if master_doc.first_reach_at and loser_doc.first_reach_at:
					dt_m = get_datetime(str(master_doc.first_reach_at))
					dt_l = get_datetime(str(loser_doc.first_reach_at))
					if dt_m and dt_l and dt_m < dt_l:
						master_older = True
				elif master_doc.first_reach_at:
					master_older = True
				elif not loser_doc.first_reach_at:
					# Both no reach date -> Use creation
					if str(master_doc.creation) < str(loser_doc.creation):
						master_older = True
...
def get_lead_province(province: str):
	...
	try:
		province_name = frappe.db.get_value("Province", {"province_name": province}, "name")
		if not province_name:
			return None
		lead_province = frappe.get_doc("Province", str(province_name))
	except Exception:
		return None
...
def update_lead_from_summary(data):
	...
	lead = cast(Lead, get_lead_by_name(lead_name))
	if not lead:
		update_contact_summary_timestamp(conversation_id)
		return
...
def auto_nurture_leads():
	...
	for lead in leads:
		...
		valid_msg_times = [dt for t in msg_times if t and (dt := get_datetime(str(t)))]

		if valid_msg_times:
			latest_msg = max(valid_msg_times)
			dt_cutoff = get_datetime(str(cutoff_time))
			if dt_cutoff and latest_msg < dt_cutoff:
				target_leads.append(lead.name)
		else:
			dt_creation = get_datetime(str(lead.creation))
			dt_cutoff = get_datetime(str(cutoff_time))
			if dt_creation and dt_cutoff and dt_creation < dt_cutoff:
				target_leads.append(lead.name)
...
def get_lead_by_conversation_id(conversation_id):
	if not conversation_id:
		cast(Any, frappe.local.response)["leads"] = []
		return
	...
	cast(Any, frappe.local.response)["leads"] = leads
	...
	for name in lead_names:
		try:
			if rule_doc:
				...
				cast(Any, rule_doc).apply_assign(doc)
			else:
