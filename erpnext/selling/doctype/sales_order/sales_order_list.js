frappe.listview_settings["Sales Order"] = {
	...
	refresh: function (listview) {
		...
		$(
			"<style>.result .list-header-subject > div:nth-child(3), .result .list-row-container .list-row-col:nth-child(3) { display: none; }</style>"
		).appendTo("head");
			...
			const observer = new MutationObserver(function (mutationsList, observer) {
				...
				for (let i = 0; i < listview.data.length; i++) {
					if (listview.data[i].cancelled_status === "Uncancelled") {
						$(`.result .list-row-container:nth-child(${i + 3}) .list-row-col:nth-child(1) a`).css(
							"color",
							"rgb(35, 98, 235)"
						);
					} else {
				...
				$("span[data-filter]")
					.removeAttr("class")
					.addClass("indicator-pill")
					.addClass("no-indicator-dot")
					.addClass("filterable");
				// Cancelled Status
				...
				for (let i = 0; i < listview.data.length; i++) {
					...
					if (row_data.is_split_order && row_data.split_order_group) {
						...
						$(
							`.result .list-row-container:nth-child(${i + 3}) .list-row-col:nth-child(1)`
						).append(badge);
					}
				}
			});
