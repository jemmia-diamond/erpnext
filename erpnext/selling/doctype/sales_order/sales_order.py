...
class SalesOrder(SellingController):
	...
	def validate(self):
		...
		self.validate_promotion_validation()
	...
	def copy_from_reference_order(self):
		...
		try:
			...
			frappe.log_error(f"Error copying from reference order: {str(e)!s}")
	...
	def copy_attachments_from_reference(self, ref_order_name):
		...
		try:
			...
			frappe.log_error(f"Error copying attachments from reference order: {str(e)!s}")
	...
	def handle_serial_numbers_changes(self):
		...
		try:
			...
			frappe.log_error(f"Error handling serial_numbers changes: {str(e)!s}")
	...
	def _get_previous_serial_numbers(self):
		...
		try:
			...
			frappe.log_error(f"Error getting previous serial_numbers: {str(e)!s}")
			return {}
	...
	def _backfill_item_from_reference_by_serial(self, current_item):
		...
		try:
			...
			frappe.log_error(f"Error backfilling item {current_item.name}: {str(e)!s}")
