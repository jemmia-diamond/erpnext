...
def get_debt_list(from_date=None, from_the_last_check=False):
	...
	sales_orders = frappe.get_all(
		"Sales Order", filters=filters, fields=fields, order_by="expected_payment_date asc"
	)
	...
	for row in sales_orders:
		...
		if key not in group_map:
			...
			if row.get("order_number"):
				if (
					not group_map[key]["order_number"]
					or row.get("order_number") < group_map[key]["order_number"]
				):
	...
	if search_codes:
		buybacks = frappe.get_all(
			"Buyback Exchange",
			filters=[["new_order_code", "in", list(search_codes)], ["status", "!=", "CANCELED"]],
	...
	from_the_last_check_bool = (
		frappe.utils.cint(from_the_last_check) or str(from_the_last_check).lower() == "true"
	)
