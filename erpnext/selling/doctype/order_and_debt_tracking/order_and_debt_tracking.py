...
class OrderandDebtTracking(Document):
	...
	def _notify_assigned_user(self):
		...
		user_info = frappe.db.get_value(
			"User", self.notify_to, ["email", "enabled", "language"], as_dict=True
		)
		...
		notification_doc = {
			...
			"subject": frappe._(
				f"Debt note on Sales Order {order_number} by {self.added_by or frappe.session.user}"
			),
