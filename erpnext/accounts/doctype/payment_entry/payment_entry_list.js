	...
	onload: function (listview) {
		...
		listview.get_call_args = function (args) {
			...
			if (custom_phone_search || custom_order_search || custom_reference_filter) {
				call_args.method =
					"erpnext.accounts.doctype.payment_entry.payment_entry.get_payment_entry_list";
		...
		if ($filter_area.length) {
			const $phone_wrapper = $(
				'<div class="form-group frappe-control input-max-width col-md-2"></div>'
			);
			const $phone_input = $(
				'<input type="text" autocomplete="off" class="input-with-feedback form-control input-xs" placeholder="' +
					__("Phone") +
					'">'
			);
			...
			const $reference_select = $(
				'<select class="input-with-feedback form-control input-xs">' +
					'<option value="" selected disabled hidden>' +
					__("Order link status") +
					"</option>" +
					'<option value="">' +
					__("All") +
					"</option>" +
					'<option value="has_references">' +
					__("Order linked") +
					"</option>" +
					'<option value="no_references">' +
					__("Not linked") +
					"</option>" +
					"</select>"
			);
			...
			const $my_entries_wrapper = $(
				'<div class="form-group frappe-control input-max-width col-md-2"></div>'
			);
			const $my_entries_checkbox = $(
				'<label class="checkbox" style="margin-top: 8px;">' +
					'<input type="checkbox" class="input-with-feedback"> ' +
					'<span class="label-area">' +
					__("My Payment Entries") +
					"</span>" +
					"</label>"
			);
			$my_entries_checkbox.find('input[type="checkbox"]').on("change", function () {
				if ($(this).is(":checked")) {
					listview.filter_area.add([
						["Payment Entry", "created_by_display", "=", frappe.session.user],
					]);
				} else {
			...
			$filter_area
				.append($my_entries_wrapper)
				.append($phone_wrapper)
				.append($order_wrapper)
				.append($reference_wrapper);
		}
	},
};
