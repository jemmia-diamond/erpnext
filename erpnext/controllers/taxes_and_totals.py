...
class calculate_taxes_and_totals:
	...
	def calculate_item_rate(self, item):
		...
		# --- PORTED CUSTOM LOGIC ---
		# if pricing rules apply OR neither rate nor discount exist, calculated rate is preferred (Jemmia Diamond)
		if has_pricing_rules or (not item.rate and not item.discount_amount):
			item.rate = calculated_rate
			return

		# Prioritize explicitly entered discount_amount over item.rate (Jemmia Diamond custom rule)
